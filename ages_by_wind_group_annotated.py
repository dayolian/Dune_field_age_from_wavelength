...
"""
# The imports are deliberately thin. This script does NOT read step 7's output
# and does not import from it. Steps 7 and 8 are siblings hanging off the same
# inputs, not a chain, so deleting step 7 would leave these numbers unchanged.
import os
import argparse
import numpy as np
import pandas as pd
...
# ---------------------------------------------------------------------------
# ANNOTATED. Your code is unchanged; everything added is a comment.
# Comments beginning with "FLAG" mark something worth acting on.
#
# This is step 8, the script the reported field ages come from.
# ---------------------------------------------------------------------------
# ---- model constants ----
MODEL_INCIP = 43.429
QS_MODEL    = 0.125
...
# All from the ReSCAL simulation, identical to step 7. W_BOUNDARY = 187.5 is the
# largest spacing the simulation actually reached; your fields sit at 800-4800,
# so every age here is read 4 to 25 times beyond the fitted range.
GREEN = "FF92D050"

# The green fill hex that marks an excluded tile. FLAG: if Excel ever re-saves
# the workbook with theme colours instead of explicit RGB, this stops matching,
# all 272 exclusions vanish silently, and the run still "succeeds". The printed
# "excluded 272 green-tagged" line is your only guard.
INTERMIT_COLS = {
...
# The four intermittency definitions step 6 wrote. net_algo is the one used;
# the other three are selectable for comparison only.
...
# Wind direction is grouped by binning a bearing to the nearest compass point.
# COMPASS_8 keeps only the 8 whose bearing divides by 45, i.e. N NE E SE S SW W NW.
...
# Snap a bearing to the nearest compass label. The min() picks whichever label
# has the smallest angular distance, using the wrap trick below so that 350 and
# 10 degrees count as 20 apart rather than 340.
def ang_diff(a, b):
...
# Smallest angle between two bearings, always 0-180. The (x+180) %% 360 - 180
# pattern folds a raw difference into that range. Used both for the agreement
# statistics and for the --max-offset filter.
...
# ^^^ THE ALIGNMENT FIX, same as step 7 but it also pulls across every
# intermittency definition plus center_algo_deg and mig_deg in one pass.
#
# The second guard is the important one: both files carry an intermittency
# value per row, so if the pairing is right those values must agree. If any
# pair differs by 0.01 or more the script RAISES instead of producing silently
# mispaired ages. That is what makes the fix trustworthy rather than plausible.
...
# Reads the physics spreadsheet. Note column I (modern_colI) is read but never
# used for anything downstream: --wavelengths is mandatory in this script, so
# the superseded pre-fix FFT values can never leak in the way they do in step 7.
def load_truth(xlsx):
...
# Reads the truth workbook for two things only: your hand migration label, and
# whether the row is green. Coordinates are rounded to 3 dp to make the join
# immune to floating-point dust.
...
# Generic lat/lon join. Pulls named columns from a CSV onto the physics table
# by matching rounded coordinates. Used for chosen_lambda_m from step 5.
...
# ^^^ THE AGE CALCULATION. Identical physics to step 7 with two differences:
#   1. use_hyst is a PARAMETER here, defaulting to OFF, whereas step 7 always
#      applies it. That alone makes step 7's ages 5x these.
#   2. interm_col is a parameter, so any of the four definitions can be used.
#
# Reading it in order:
#   l0    metres per model block at this site, from incipient wavelength
#   t0    years per model timestep, from the site flux. Small flux -> large t0
#   W     measured spacing expressed in model blocks
#   N_pow / N_lin  the growth laws rearranged to give timesteps from W
#   age   t0 * N / intermittency. Dividing by intermittency because sand only
#         moves during that fraction of hours, so low intermittency needs more
#         elapsed years for the same transport.
#
# N_lin goes negative when W < 110.2. That is White Sands, at W = 71: not a
# wrong age, an undefined one.
# N_flo is a CONSTANT, so the floor age carries no wavelength information at
# all. Already dropped from the published comparison for that reason.
def geomean(v):
...
# Two summary statistics, both ignoring non-positive and non-finite values so a
# single negative tile cannot poison a group.
#
# geomean is the one that matters: it averages by RATIO rather than by
# difference, which is correct for quantities spanning orders of magnitude.
# Averaging 100 and 10,000 gives 1,000 rather than 5,050.
...
# One row per (field, wind direction). Reports every growth law both ways so
# the choice of statistic is auditable. The suffix argument lets the same
# function build the parallel "_allpts" columns from the unfiltered tiles.
...
# ^^^ ONE AGE PER FIELD. Two stages: geomean within each wind group, then a
# size-weighted geomean across groups, weighted in log space.
#
# Because the weights are group sizes, this is algebraically identical to the
# geomean over all tiles WHENEVER no group is dropped. With min_n = 1 nothing
# is dropped, so the grouping does not change the field number at all. It earns
# its place by exposing within-field structure, not by altering the answer.
#
# "used = kept if kept else grp" is the fallback: if no group clears min_n, use
# all of them rather than returning nothing. thin_fallback records when that
# happened, which is how Lencois and White Sands get flagged.
...
# ^^^ THE VALIDATION RESULT. Compares where ERA5 pushes the dunes against where
# you measured them moving, for the tiles where you could measure it. The 42
# degree median and 87 percent within 90 come from here.
#
# Ignore the "same" column: an 8-point bin is 45 degrees wide, so a 42 degree
# median straddles a boundary by construction and that column mostly measures
# bin luck. The offset percentiles are the real numbers.
def main():
    ...
    # ---- STAGE 2: attach measured wavelengths ----
    # FLAG, and a good one: unlike step 7 this REFUSES to run without the FFT
    # file rather than quietly falling back to superseded column I. This single
    # raise is the main reason step 8 is the trustworthy script.
    ...
    # ---- STAGE 3: work out which way the wind pushes each tile ----
    # center_algo_deg is where the wind blows FROM, so +180 gives where it
    # pushes sand TO. That bearing is then binned to a compass label, and the
    # label is what tiles are grouped on.
    ...
    # Ratio of oldest to youngest wind group within each field. This is what
    # revealed that Chad spans 10.8x internally, and it is arguably more
    # interesting than the field averages themselves.
