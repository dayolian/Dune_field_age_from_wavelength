...
# ----------------- USER SETTINGS -----------------
image_folder = Path("model_images")   # folder with PNGs
output_folder = Path("fft_summary_tight_presentation")   # where to save figures
...
def make_summary_figure(
    ...
    """
    Make 4-panel figure and save to outpath, using fixed axis limits.
    Bottom-right panel is a polar rose diagram of angular power.
    """
    ...
    ax_rad = fig.add_subplot(2, 2, 3)
    ax_ang = fig.add_subplot(2, 2, 4, projection="polar")

    # --- Cropped image ---
    ax_img.imshow(img, cmap="gray", origin="upper")
    ax_img.set_title("Model dunes")
    ax_img.set_xticks([])
    ax_img.set_yticks([])
    ...
    # --- Polar rose diagram (orientation vs power) ---
    # theta_deg is 0–180° orientation; mirror into 180–360° to show symmetry
    theta_rad = np.deg2rad(theta_deg)
    theta_full = np.concatenate([theta_rad, theta_rad + np.pi])
    power_full = np.concatenate([power_theta, power_theta])

    # Bar width based on bin spacing
    if len(theta_deg) > 1:
        dtheta = np.deg2rad(theta_deg[1] - theta_deg[0])
    else:
        dtheta = np.deg2rad(180.0 / max(len(theta_deg), 1))
    ...
    ax_ang.set_title("Angular spectrum (rose diagram)")
    ax_ang.set_rmax(ang_ylim[1])
    ax_ang.set_rticks([])  # optional: hide radial tick labels
    ax_ang.set_theta_zero_location("N")  # 0° to the right
    ax_ang.set_theta_direction(-1)       # angles increase clockwise

    # Overall title
    #if time_k is not None:
    #    fig.suptitle(f"{filename}   (t = {time_k} × 1000 t₀)", fontsize=12)
    #else:
